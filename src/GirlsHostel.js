...
import './Hostel.css'
function GirlsRoom(props) {

    let [girlRoom, setGirlRoom] = useState("");
    let [floor, setFloor] = useState("");
    //let hostel = props.hostel

    // localStorage.setItem('')
    function selectRoom(e) {
        setGirlRoom(e.target.value)
        console.log(girlRoom)
        localStorage.setItem('roomNo', girlRoom)
    ...
    return (
        (floor) ?
            <Booked value1={girlRoom} Name={props.Name} Photo={props.Photo} value2={props.hostel} girl={true} /> :
            <>
                <Topbar Name={props.Name} Photo={props.Photo} />
                <div className="Hostel">
                    <div className="roomBox">
                        <h2>Girls Hostel: Choose a Floor of G{props.hostel}</h2>
                        <select id="floor">
